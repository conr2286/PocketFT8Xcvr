...
Without the GPS, the transmitter uses the manually configured locator, and restores the date and time from Teensy's battery-backed clock.
Without the GPS, the station's timezone is unknown and QSOs may not be logged using UTC time (required by ADIF standards).
...
The transceiver's user interface consists of a waterfall display of activity within the receiver's passband, a status display, a display of decoded messages, a transcript of the current QSO's traffic (if any), and a set of menu buttons.
...
The limited display size requires the signal strength of the decoded messages to appear as simple S values (e.g. S1, S2, ... S9) rather than dB as in WSJTX.
...
RoboOp (the firmware sequencing a WSJTX-style QSO) will begin transmitting at the next appropriate FT8 timeslot, avoiding those timeslots when a called station will likely transmit.
...
In all cases, RoboOp attempts to avoid transmitting in the same timeslot as a specified remote is expected to transmit.
