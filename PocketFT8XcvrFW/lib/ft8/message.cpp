...
#include <stdlib.h>
#include <string.h>
#include "DEBUG.h"  //Pocket FT8 Debugging macros
 ...
 * @param result Array to receive the packed message
 * @param hash_if Ignored (can be NULL)
 * @param message_text Message to be packed
 * @return 0==success
 */
ftx_message_rc_t ftx_message_encode(ftx_message_t* result, ftx_callsign_hash_interface_t* hash_if, const char* message_text) {
    char call_to[12];
    char call_de[12];
    char extra[20];
    ...
    if (is_cq) {
        ...
        DPRINTF("next token after CQ: '%s' in '%s'\n", call_de, message_text);
        ...
        if (cq_modifier_v >= 0) {
            ...
            DPRINTF("CQ modifier encoding %d; parse_pos after CQ: %s in %s\n", cq_modifier_v, parse_position, message_text);
        } else {
    ...
    DPRINTF("ftx_message_encode: parsed '%s' '%s' '%s'; remaining chars '%s'\n", call_to, call_de, extra, parse_position);
    ...
    if (!parse_position[0]) {
        ...
        rc = ftx_message_encode_std(result, hash_if, call_to, call_de, extra);
        if (rc == FTX_MESSAGE_RC_OK)
            return rc;
        DPRINTF("   ftx_message_encode_std failed: %d\n", rc);
        rc = ftx_message_encode_nonstd(result, hash_if, call_to, call_de, extra);
        if (rc == FTX_MESSAGE_RC_OK)
            return rc;
        DPRINTF("   ftx_message_encode_nonstd failed: %d\n", rc);
    }
    rc = ftx_message_encode_free(result, message_text);
    if (rc == FTX_MESSAGE_RC_OK)
        return rc;
    DPRINTF("   ftx_message_encode_free failed: %d\n", rc);

    return rc;
}  // ftx_message_encode()

/**
 * @brief Pack standard message text into a 77-bit buffer
 * @param result Buffer to receive the packed message
 * @param hash_if Ignored
 * @param call_to Destination station's callsign
 * @param call_de Source station's callsign
 * @param extra Field3 (e.g. 73, R73, RR73, locator...)
 * @return 0==success
 */
ftx_message_rc_t ftx_message_encode_std(ftx_message_t* result, ftx_callsign_hash_interface_t* hash_if, const char* call_to, const char* call_de, const char* extra) {
    uint8_t ipa, ipb;

    DPRINTF("ftx_message_encode_std '%s' '%s' '%s'\n", call_to, call_de, extra);

    int32_t n28a = pack28(call_to, hash_if, &ipa);
    int32_t n28b = pack28(call_de, hash_if, &ipb);
    DPRINTF("   n29a = %d, n29b = %d\n", n28a, n28b);
    ...
    result->payload[0] = (uint8_t)(n29a >> 21);
    result->payload[1] = (uint8_t)(n29a >> 13);
    result->payload[2] = (uint8_t)(n29a >> 5);
    result->payload[3] = (uint8_t)(n29a << 3) | (uint8_t)(n29b >> 26);
    result->payload[4] = (uint8_t)(n29b >> 18);
    result->payload[5] = (uint8_t)(n29b >> 10);
    result->payload[6] = (uint8_t)(n29b >> 2);
    result->payload[7] = (uint8_t)(n29b << 6) | (uint8_t)(igrid4 >> 10);
    result->payload[8] = (uint8_t)(igrid4 >> 2);
    result->payload[9] = (uint8_t)(igrid4 << 6) | (uint8_t)(i3 << 3);

    return FTX_MESSAGE_RC_OK;
}  // ftx_message_encode_std()

/**
 * @brief Pack a nonstandard message text into a 77-bit buffer
 * @param result Buffer to receive the packed message
 * @param hash_if Ignored
 * @param call_to Destination callsign
 * @param call_de Source callsign
 * @param extra Field3 (73, R73 or RR73)
 * @return 0==success
 *
 * @note An FT8 non-standard messae does not include a locator
 */
ftx_message_rc_t ftx_message_encode_nonstd(ftx_message_t* result, ftx_callsign_hash_interface_t* hash_if, const char* call_to, const char* call_de, const char* extra) {
    uint8_t i3 = 4;

    DPRINTF("ftx_message_encode_nonstd '%s' '%s' '%s'\n", call_to, call_de, extra);
    ...
    if (icq == 0) {
        ...
        n12 = 0;
        call58 = call_de;
        DPRINTF("CQ: 58-bit '%s'; omitting grid '%s'\n", call58, extra);
    }
    ...
    result->payload[0] = (uint8_t)(n12 >> 4);
    result->payload[1] = (uint8_t)(n12 << 4) | (uint8_t)(n58 >> 54);
    result->payload[2] = (uint8_t)(n58 >> 46);
    result->payload[3] = (uint8_t)(n58 >> 38);
    result->payload[4] = (uint8_t)(n58 >> 30);
    result->payload[5] = (uint8_t)(n58 >> 22);
    result->payload[6] = (uint8_t)(n58 >> 14);
    result->payload[7] = (uint8_t)(n58 >> 6);
    result->payload[8] = (uint8_t)(n58 << 2) | (uint8_t)(iflip << 1) | (uint8_t)(nrpt >> 1);
    result->payload[9] = (uint8_t)(nrpt << 7) | (uint8_t)(icq << 6) | (uint8_t)(i3 << 3);

    return FTX_MESSAGE_RC_OK;
}  // ftx_message_encode_nonstd()

/**
 * @brief Pack a 13-char free text message into a 77-bit buffer
 * @param result Buffer to receive the packed message
 * @param text The free text message to pack
 * @return 0==success
 */
ftx_message_rc_t ftx_message_encode_free(ftx_message_t* result, const char* text) {
    ...
    ftx_message_rc_t ret = ftx_message_encode_telemetry(result, b71);
    result->payload[9] = 0;  // i3.n3 = 0.0; etc.
    return ret;
}  // ftx_message_encode_free()

// TODO set byte 9? or must the caller do it?

/**
 * @brief Pack a telemetry message into a 77-bit buffer
 * @param result Buffer to receive the packed message
 * @param telemetry The telemetry text
 * @return 0==OK
 *
 * TODO set byte 9? or must the caller do it?
 *
 * @note Does anyone know what this is for???
 */
ftx_message_rc_t ftx_message_encode_telemetry(ftx_message_t* result, const uint8_t* telemetry) {
    // Shift bits in telemetry left by 1 bit to right-align the data
    uint8_t carry = 0;
    for (int i = 8; i >= 0; --i) {
        result->payload[i] = (telemetry[i] << 1) | (carry >> 7);
        carry = telemetry[i] & 0x80;
    }
    return FTX_MESSAGE_RC_OK;
}  // ftx_message_encode_telemetry()

/**
 * @brief Unpack a 77-bit FT8 message
 * @param msg Bfr containing the 77-bit message to unpack
 * @param hash_if Ignored
 * @param message Buffer to receive the unpacked message
 * @param offsets The message[] indices for FIELD1, FIELD2 and FIELD3
 * @return 0==OK
 *
 * @note Supports only the FT8 standard, nonstandard call, free text and telemetry messages
 */
ftx_message_rc_t ftx_message_decode(const ftx_message_t* msg, ftx_callsign_hash_interface_t* hash_if, char* message, ftx_message_offsets_t* offsets) {
    ...
    char* field1 = buf;  // All three NUL-terminated fields get unpacked into one buf[]
    char* field2 = buf + 14;
 ...
 * @param msg Bfr containing the 77-bit message to unpack
 * @param hash_if Ignored
 * @param field1 14-char bfr to receive field1
 * @param field2 14-char bfr to receive field2
 * @param field3 14-char bfr to receive field3
 * @param message 35-char bfr to receive complete unpacked message
 * @param offsets The types and offsets of the fields in message
 * @return 0==OK
 *
 * @note Supports only the FT8 standard, nonstandard call, free text and telemetry messages
 */
ftx_message_rc_t ftx_message_decode(const ftx_message_t* msg, ftx_callsign_hash_interface_t* hash_if, char* message, char* field1, char* field2, char* field3, ftx_message_offsets_t* offsets) {
    ftx_message_rc_t rc;
...
/**
 * @brief Unpack an FT8 standard 77-bit message
 * @param msg Buffer containing the 77-bit message to unpack
 * @param hash_if Ignored
 * @param call_to Destination station's callsign field
 * @param call_de Source station's callsign field
 * @param extra Extra (e.g. 73, R73, etc) field
 * @param field_types Identifies the type of each of the three fields
 * @return 0==success
 */
ftx_message_rc_t ftx_message_decode_std(const ftx_message_t* msg, ftx_callsign_hash_interface_t* hash_if,
    ...
    // Extract i3 (bits 74..76)
    uint8_t i3 = (msg->payload[9] >> 3) & 0x07u;
    DPRINTF("decode_std() n28a=%d ipa=%d n28b=%d ipb=%d ir=%d igrid4=%d i3=%d\n", n29a >> 1, n29a & 1u, n29b >> 1, n29b & 1u, ir, igrid4, i3);
    ...
    DPRINTF("Decoded standard (type %d) message [%s] [%s] [%s]\n", i3, call_to, call_de, extra);
    return FTX_MESSAGE_RC_OK;
}  // ftx_message_decode_std()

/**
 * @brief Unpack a non-standard call message
 * @param msg The 77-bit buffer containing the packed message
 * @param hash_if Ignored
 * @param call_to Destination station's callsign field1
 * @param call_de Source station's callsign field2
 * @param extra Field3 (e.g. R73, RR73, 73)
 * @param field_types Identifies the type of each of the three fields
 * @return 0==success
 *
 * @author KD8CEC and others
 */
ftx_message_rc_t ftx_message_decode_nonstd(const ftx_message_t* msg, ftx_callsign_hash_interface_t* hash_if,
    ...
    // Extract i3 (bits 74..76)
    uint8_t i3 = (msg->payload[9] >> 3) & 0x07u;
    DPRINTF("decode_nonstd() n12=%04x n58=%08llx iflip=%d nrpt=%d icq=%d i3=%d\n", n12, n58, iflip, nrpt, icq, i3);
    ...
    DPRINTF("Decoded non-standard (type %d) message [%s] [%s] [%s]\n", i3, call_to, call_de, extra);
    return FTX_MESSAGE_RC_OK;
}  // ftx_message_decode_nonstd()

/**
 * @brief Unpack a free text message
 * @param msg Includes the 77-bit message to unpack
 * @param text Buffer to receive the unpacked message
 */
...
}  // ftx_message_decode_free()

/**
 * @brief Unpack a telemetry hex message (Don't know much about these;)
 * @param result
 * @param telemetry_hex
 */
void ftx_message_decode_telemetry_hex(const ftx_message_t* result, char* telemetry_hex) {
    uint8_t b71[9];

    ftx_message_decode_telemetry(result, b71);
    ...
    telemetry_hex[18] = '\0';
}  // ftx_message_decode_telemetry_hex()

/**
 * @brief Does anyone know what these telemetry messages are used for?
 * @param result
 * @param telemetry
 */
void ftx_message_decode_telemetry(const ftx_message_t* result, uint8_t* telemetry) {
    // Shift bits in payload right by 1 bit to right-align the data
    uint8_t carry = 0;
    for (int i = 0; i < 9; ++i) {
        telemetry[i] = (carry << 7) | (result->payload[i] >> 1);
        carry = (result->payload[i] & 0x01u);
    }
}  // ftx_message_decode_telemetry()
...
/**
 * @brief Save a callsign in the callsign map
 * @param hash_if Ignored
 * @param callsign Callsign to save
 * @param n22_out NULL or receives 22-bit hash key
 * @param n12_out NULL or receives 12-bit hash key
 * @param n10_out NULL or receives 10-bit hash key
 * @return true==success
 */
static bool save_callsign(const ftx_callsign_hash_interface_t* hash_if, const char* callsign, uint32_t* n22_out, uint16_t* n12_out, uint16_t* n10_out) {
    uint64_t n58 = 0;
    ...
    uint32_t n12 = n22 >> 10;
    uint32_t n10 = n22 >> 12;
    DPRINTF("save_callsign('%s') = [n22=%d, n12=%d, n10=%d]\n", callsign, n22, n12, n10);
...
/**
 * @brief Retrieve callsign from map using an FT8 hash key
 * @param hash_if Ignored
 * @param hash_type Identifies #bits in hash key
 * @param hash The hash key
 * @param callsign Buffer for retrieved callsign
 * @return true==success
 */
static bool lookup_callsign(const ftx_callsign_hash_interface_t* hash_if, ftx_callsign_hash_type_t hash_type, uint32_t hash, char* callsign) {
    char c11[12];
...
static int parse_cq_modifier(const char* string) {
    ...
    DPRINTF("CQ_nnn/CQ_a[bcd] '%s' %d/%d\n", string, nnum, nlet);
    if (nnum == 3 && nlet == 0) {
        DPRINTF("CQ_nnn detected: %d\n", atoi(string + 3));
        return atoi(string + 3);
    } else if (nnum == 0 && nlet <= 4) {
        DPRINTF("CQ_a[bcd] detected: m %d\n", m);
        return 1000 + m;
    }
...
static int32_t pack28(const char* callsign, const ftx_callsign_hash_interface_t* hash_if, uint8_t* ip) {
    ...
    *ip = 0;
    ...
    if (ends_with(callsign, "/P") || ends_with(callsign, "/R")) {
        DPRINTF("Suffix /P or /R detected\n");
        *ip = 1;
    ...
    if ((length >= 3) && (length <= 11)) {
        ...
        DPRINTF("Encoding '%s' as non-standard callsign\n", callsign);
        uint32_t n22;
